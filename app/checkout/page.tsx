...
// Type-cast Checkout component to accept onSuccess
const CheckoutWithSuccess = Checkout as ComponentType<{
  items: any[]
  onSuccess?: () => void
}>
...
export default function CheckoutPage() {
  ...
  const completeOrder = () => {
    ...
    const purchasedServices = items.map((item) => {
      ...
      return {
        ...
        datePurchased: new Intl.DateTimeFormat('en-GB', {
          dateStyle: 'medium',
        }).format(new Date()),
    ...
    addPurchasedServices(purchasedServices)
    clear()
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-8">
          ...
          <p className="mt-3 max-w-2xl text-slate-600">
            Pay securely by card or another eligible payment method. Audvetax checkout is powered by Stripe.
          </p>
        </div>
        ...
        <div className="mt-8 rounded-2xl bg-white p-3 shadow-xl sm:p-6">
          {items?.length ? (
            <>
              <CheckoutWithSuccess items={items} onSuccess={() => setIsPaid(true)} />
              {isPaid && (
                <button
