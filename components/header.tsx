        ...
        <div className="mx-auto max-w-7xl px-4 py-2 sm:px-6 sm:py-4 lg:px-8">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-4">
              <CartLink />
              {isLoggedIn ? <div className="flex items-center gap-2"><Link href="/dashboard" className="rounded border-2 border-cyan-400 px-4 py-2 text-sm font-bold text-slate-950 transition hover:bg-cyan-50">Dashboard</Link><button onClick={() => { logout(); router.push('/') }} className="text-sm font-semibold text-slate-600 hover:text-slate-950">Sign out</button></div> : <Link href="/login" className="block rounded border-2 border-gray-900 px-4 py-2 text-sm font-medium text-gray-900 transition hover:bg-gray-50">{t('clientLogin')}</Link>}
            </div>
